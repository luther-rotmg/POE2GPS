...
| 13 | **Cloudflare Worker prod deploy for v0.21** | `CF-DASH-BUTTONS` app-build ship — stale desktop clients hitting new routes before deploy get clean 404 rather than schema-mismatch 400 | Run `wrangler deploy` against the v0.21 `worker.js` after `CF-WORKER` merges to main; verify smoke script hits 200 on `/submit-atlas` and 429 after 5 rapid POSTs from a single IP | 10 min | Ship of `CF-DASH-BUTTONS` PR (v0.21 spec §5 ordering) |
