...
///
/// v0.33 refactor (#29a): the CSS body has been extracted to Web/Assets/dashboard.css and
/// is spliced back into <see cref="Page"/> at first access via <see cref="AssemblePage"/>.
/// Output is byte-for-byte identical to the pre-refactor const-string. #29b will do the same
/// for the JS body; #29c will retire the raw literal entirely.
/// </summary>
internal static class DashboardHtml
{
    ...
    // v0.33 #29a: raw page with CSS extracted to /*DASHBOARD_STYLE_INLINE_v1*/ sentinel.
    // Access via <see cref="Page"/> — <see cref="AssemblePage"/> splices the CSS back in.
    private const string _pageRaw = """
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<style>/*DASHBOARD_STYLE_INLINE_v1*/</style>
</head>
<body>
...
""";
    ...
    // v0.33 #29a: byte-exact page assembly. AssemblePage runs ONCE at first Page access
    // (Lazy<T> + PublicationOnly — no init-time exception latching, no lock convoy).
    // The sentinel is replaced with the raw bytes of the embedded dashboard.css; no
    // wrapper whitespace / newlines / <style> tags are emitted, so output is byte-for-byte
    // identical to the pre-refactor const-string. #29b will splice a JS sentinel too.
    ...
    private static string AssemblePage()
    {
        ...
        return _pageRaw.Replace("/*DASHBOARD_STYLE_INLINE_v1*/", css);
    }

    /// <summary>Read an embedded resource as UTF-8, stripping a leading BOM if present.
    /// Throws <see cref="InvalidOperationException"/> with a descriptive message if the
    /// manifest resource is missing — surfaces registration mistakes at first Page access
    /// instead of silently serving a broken document.</summary>
    private static string LoadEmbeddedText(string resourceName, string friendlyName)
    {
        ...
        // Strip UTF-8 BOM (EF BB BF) if present — Encoding.UTF8.GetString does not.
