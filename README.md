...
- 🔮 **Preload Alert** *(opt-in, off by default · experimental)* — on zone entry, reads the asset list the game just loaded and surfaces the **notable content in the zone** — pinnacle bosses, league encounters (Breach · Ritual · Expedition · Abyss · Incursion · Delirium), Rogue/Conqueror exiles, valuable chests — as a tier-coloured corner panel, with an optional audio cue. A **self-tuning noise filter** learns and suppresses the always-loaded base assets so only *this zone's* content shows, and a **Diagnostic view** lets you tune it. **Read-only — only reads what the game already loaded, never sends input.**
